...
function truncateLabel(text, maxWidth) {
  let w = 0, i = 0;
  for (; i < text.length; i++) w += text.charCodeAt(i) > 255 ? 2 : 1;
  if (w <= maxWidth) return text;
  w = 0;
  for (i = 0; i < text.length; i++) {
    const cw = text.charCodeAt(i) > 255 ? 2 : 1;
...
class TopoViz {
  ...
  _node(x, y, r, fill, label, id, glowColor, titleText) {
    ...
    if (titleText) {
      const tt = document.createElementNS(this.svgNS, 'title');
  ...
  addSubagent(id, topic) {
    ...
    const x = 80 + col * 220;
    const y = 150 + row * 110;
    const o = this._node(x, y, 14, '#103a5c', truncateLabel(topic, 14), id, '#4f8cff', topic);
    // 主→子 边
