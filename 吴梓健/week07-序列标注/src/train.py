...
from dataset import build_label_schema_cluener,build_label_schema_people_daily, build_dataloaders
...
def main():
    ...
    if args.dataset == 'cluener':
        print("使用数据集：CLUENER")
        DATA_DIR = ROOT / "data" / "cluener"
        labels, label2id, id2label = build_label_schema_cluener()
        num_labels = len(labels)
        print(f"BIO 标签数：{num_labels}（O + {len(labels) - 1} 个实体标签）")
    elif args.dataset == 'peoples_daily':
        print("使用数据集：Peoples Daily")
        DATA_DIR = ROOT / "data" / "peoples_daily"
        labels, label2id, id2label = build_label_schema_people_daily(data_dir=DATA_DIR)
        num_labels = len(labels)
        print(f"BIO 标签数：{num_labels}（O + {len(labels) - 1} 个实体标签）")
    else:
        raise ValueError(f"不支持的数据集名称：{args.dataset}（请选择 'cluener' 或 'peoples_daily'）")
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default='peoples_daily', help="数据集名称（支持 cluener、peoples_daily）")
