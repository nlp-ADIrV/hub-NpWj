...
def build_label_schema_cluener() -> tuple[list[str], dict[str, int], dict[int, str]]:
    """cluener2020 构建 BIO 标签体系，返回 (labels, label2id, id2label)。"""
    labels = ["O"]
...
def build_label_schema_people_daily(data_dir: Path) -> tuple[list[str], dict[str, int], dict[int, str]]:
    """people_daily 构建 BIO 标签体系，返回 (labels, label2id, id2label)。"""
    # 读取label_names.json文件，获取实体类型列表
...
class PeoplesDailyDataset(Dataset):
    """Peoples Daily 的 PyTorch Dataset。"""
    # 该数据集已经是 BIO 格式，且每行一个字符及其标签
    def __init__(
        self,
        records: list,
        tokenizer: BertTokenizer,
    ...
    def __getitem__(self, idx: int) -> dict:
        row = self.records[idx]
        tokens = row["tokens"]
        ner_tags = row.get("ner_tags") or row.get("labels") or []
...
def build_dataloaders(
    ...
    if data_dir and "peoples_daily" in str(data_dir):
