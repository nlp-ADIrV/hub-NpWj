...
教学重点：
  ...
  2. 多轮循环闭环：模型输出 tool_call → 宿主执行工具 → 结果以 role=tool 回填 → 模型继续输出…
     循环往复，直到模型不再输出 tool_call，给出最终回答
...
from src.weather_backend import get_city_location, get_weather_by_location  # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "根据城市名获取城市经纬度等地理信息（纬度 latitude、经度 longitude、"
                "城市名 city_name、国家 country、省份 admin1）。"
                "城市用中文名，如 '宁德'、'北京'。返回 JSON 字符串。"
                "查天气时需先调用本工具获取经纬度，再调用 get_weather_by_location。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "根据经纬度查询当前天气及未来3天预报。"
                "需要先调用 get_city_location 获取经纬度后再调用本工具。"
                "可选传入 city_name/country/admin1 用于结果展示。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "latitude": {"type": "number", "description": "纬度，如 26.66"},
                    "longitude": {"type": "number", "description": "经度，如 119.55"},
                    "city_name": {"type": "string", "description": "城市名（仅用于展示，可选）"},
                    "country": {"type": "string", "description": "国家（仅用于展示，可选）"},
                    "admin1": {"type": "string", "description": "省/州（仅用于展示，可选）"},
                },
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时需分两步：先调用 get_city_location 获取城市经纬度，"
    "再调用 get_weather_by_location 用经纬度查询天气。"
    "你可以一次调用多个工具，也可以多轮调用工具直到获取到所有需要的信息。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    多轮循环闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 模型继续输出…
    不断循环，直到模型不再输出 tool_call，给出最终回答。
    ...
    # 【教学时刻 3】：循环调用工具，直到模型不再输出 tool_call
    # 每轮：模型决定是否调用工具 → 有 tool_call 就执行并回填 → 继续下一轮
    # 无 tool_call 则模型已给出最终回答，退出循环
    MAX_ROUNDS = 10  # 安全阀：防止模型陷入无限循环
    msg = None
    for round_idx in range(1, MAX_ROUNDS + 1):
        ...
        # 模型没有输出 tool_call → 已生成最终回答，退出循环
        if not msg.tool_calls:
            break

        if verbose:
            print(f"  → [round {round_idx}] 模型请求调用 {len(msg.tool_calls)} 个工具")
        ...
        # 继续下一轮，让模型看到工具结果后决定是否再次调用工具
    else:
        # 达到最大轮次仍有 tool_call，强制退出
        print(f"  → [warn] 达到最大调用轮次 {MAX_ROUNDS}，强制结束")
