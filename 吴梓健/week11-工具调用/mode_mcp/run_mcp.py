...
教学重点：
  ...
  本文件的多轮循环代码和 run_function_call.py 几乎一样——差异只在"工具从哪来/怎么执行"
...
async def run(client, model: str, question: str,
    ...
    """多轮循环闭环：提问 → 模型输出 tool_call → 路由到 Server 执行 → 回填 → 模型继续输出…
...
async def main_async(provider: str, question: str | None, demo: bool, verbose: bool, as_json: bool):
    ...
    async with AsyncExitStack() as stack:
        ...
        print(json.dumps(openai_tools, ensure_ascii=False),'\n')
