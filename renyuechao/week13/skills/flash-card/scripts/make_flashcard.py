...
def render_examples(examples):
    ...
    for example in fixed:
        english = html.escape(example.get("en", "") or "（待补充例句）")
        chinese = html.escape(example.get("zh", "") or "（待补充翻译）")
        items.append(
            f'<li><div class="en">{english}</div>'
            f'<div class="zh">{chinese}</div></li>'
        )
    return "\n        ".join(items)
...
def main():
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path.cwd() / f"{data['word']}.html"
    )
