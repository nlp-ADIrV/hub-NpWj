...
from src.skill_router import detect_skill, load_skill_content
...
# ── 渐进式加载的技能（工具型）触发词 ────────────────────────────────
TOOL_SKILL_TRIGGERS = [
    "闪卡", "flash card", "flashcard", "单词卡", "单词卡片",
    "画个图", "画一张", "画一个", "架构图", "流程图", "时序图",
    "结构图", "思维导图", "diagram", "flowchart", "sequence diagram", "svg",
]


def looks_like_tool_skill(text: str) -> bool:
    """启发式：判断输入是否像在请求某个工具型技能（渐进式加载路由用）"""
    t = text.lower()
    return any(k in t for k in TOOL_SKILL_TRIGGERS)


def run_skills_agent(query: str, db: SessionDB, session_id: int, messages: list[dict]):
    """启动渐进式技能 Agent（ReAct 循环），并把问答写回会话数据库。"""
    from src.skills_agent import SkillsAgent
    agent = SkillsAgent()
    print(f"\n{MAGENTA}{'═'*60}{RESET}")
    print(f"{MAGENTA}  渐进式技能 Agent — ReAct 循环{RESET}")
...
def print_layer_info(layers, semantic_results=None):
    ...
    # skill 层名自动从 label 推断
    for layer in layers:
        if layer.name not in layer_names:
            layer_names[layer.name] = layer.source
    for layer in layers:
...
def main():
    ...
    print("输入 /flush, /memory, /layers, /new, /skill <任务>, /exit 查看各功能")
    print(f"{DIM}技能：含「张雪峰」→ 人格技能全量注入；含「闪卡/画个图/diagram…」或 /skill → 渐进式加载 ReAct Agent{RESET}\n")
    ...
    while True:
        ...
        if user_input == "/skills":
            from src.skill_router import SKILLS
            print(f"\n{CYAN}可用技能：{RESET}")
            for name, config in SKILLS.items():
                keywords = ", ".join(config["keywords"][:3])
                print(f"  🎭 {config['name']}  {DIM}关键词: {keywords}...{RESET}")
        ...
        if user_input.startswith("/skill "):
            query = user_input[len("/skill "):].strip()
            if not query:
                print(f"{YELLOW}用法：/skill 你的任务描述，例如 /skill 给我做一张 crazy 的闪卡{RESET}")
            else:
                run_skills_agent(query, db, session_id, messages)
            continue
        ...
        if skill_name:
            skill_layers = load_skill_content(skill_name)
            char_total = sum(s["char_count"] for s in skill_layers)
            print(f"  {YELLOW}[🎭 技能激活] {skill_name} ({char_total} 字符){RESET}")

        # ── 渐进式加载路由（工具型技能）────────────────────────────────
        if not skill_name and looks_like_tool_skill(user_input):
            print(f"  {YELLOW}[⚙️ 检测到工具型技能请求，转入渐进式加载 Agent]{RESET}")
            run_skills_agent(user_input, db, session_id, messages)
            continue
        ...
        if skill_layers:
            # 重新构建 system prompt 带上 skill
            prompt_result = loader.build_system_prompt(
                recent_memory_limit=10, skill_layers=skill_layers)
            system_prompt = prompt_result.system_prompt
        if semantic_context:
            system_prompt += f"\n\n## 语义检索到的相关记忆\n{semantic_context}"
