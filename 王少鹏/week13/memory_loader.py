...
教学重点：
  ...
  4. Skill 扩展：可动态加载技能 SKILL.md 叠加到 system prompt
...
class MemoryLoader:
    ...
    def build_system_prompt(self, recent_memory_limit: int = 10,
                             skill_layers: list[dict] | None = None) -> SystemPromptResult:
        """
        四层拼接：SOUL → USER → AGENTS → 近期 MEMORY 条目
        可选 skill_layers：外部传入的技能内容列表（来自 skill_router.load_skill_content）
        ...
        # 扩展：Skill 层（动态加载的技能内容）
        if skill_layers:
            for sl in skill_layers:
                section = f"## {sl['label']}\n\n{sl['content']}"
                layers.append(MemoryLayer(sl["name"], sl["label"], section))
                parts.append(section)
