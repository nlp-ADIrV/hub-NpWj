...
def build_client() -> OpenAI:
    ...
    if not api_key:
        try:
            ...
            api_key = None
    if not api_key:
        raise EnvironmentError("请设置环境变量 DEEPSEEK_API_KEY，或用 setx 设置后重启终端再运行")
    return OpenAI(
        api_key=api_key,
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """从 peoples_daily BIO 格式提取 gold spans。"""
    ...
    while i < len(tokens):
        ...
        if tag.startswith("B-"):
            ...
            j = i + 1
            while j < len(tokens) and ner_tags[j] == f"I-{etype}":
                j += 1
            surface = "".join(tokens[i:j])
            spans.add((surface, etype, start, start + len(surface) - 1))
            i = j
        else:
            i += 1
    return spans
...
FEW_SHOT_EXAMPLES = [
    {
        ...
        "output": '{"entities": [{"text": "新华社", "type": "ORG"}, {"text": "北京", "type": "LOC"}, {"text": "国务院", "type": "ORG"}, {"text": "李岚清", "type": "PER"}, {"text": "中南海", "type": "LOC"}, {"text": "美国前商务部", "type": "ORG"}, {"text": "芭芭拉·弗兰克林", "type": "PER"}]}'
    },
    {
        "text": "在卢森堡，金融分支机构从它获得经营许可的第一天起，就开始受到货币局的严格监督。",
        "output": '{"entities": [{"text": "卢森堡", "type": "LOC"}, {"text": "货币局", "type": "ORG"}]}'
    },
    {
