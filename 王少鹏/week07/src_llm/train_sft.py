    ...
    while i < len(tags):
        ...
        if tag.startswith("B-"):
            ...
            while i < len(tags) and tags[i] == f"I-{etype}":
                i += 1
            entities.append({"text": "".join(tokens[start:i]), "type": etype})
        else:
            entities.append({"text": tokens[i], "type": tag[2:]})
            i += 1
    return entities
...
def record_to_target(record: dict) -> str:
    ...
    输入：{"tokens": ["中", "国"], "ner_tags": ["B-LOC", "I-LOC"]}
    输出：'{"entities": [{"text": "中国", "type": "LOC"}]}'
    """
    entities = parse_bio(record["tokens"], record["ner_tags"])
