...
def fix_illegal_bio(seq: list[str]) -> list[str]:
    """后处理修复非法 BIO 转移：
      - I-X 开头 → B-X
      - O → I-X → O → B-X
      - B-X → I-Y 或 I-X → I-Y (X≠Y) → B-Y
    """
    fixed = list(seq)
    if fixed and fixed[0].startswith("I-"):
        fixed[0] = "B-" + fixed[0][2:]
    for i in range(1, len(fixed)):
        prev, curr = fixed[i - 1], fixed[i]
        if curr.startswith("I-") and prev.startswith("I-"):
            prev_type, curr_type = prev[2:], curr[2:]
            if prev_type != curr_type:
                fixed[i] = "B-" + curr_type
    # 第二遍：处理 O→I-X 可能被上一遍遗漏的
    for i in range(1, len(fixed)):
        prev, curr = fixed[i - 1], fixed[i]
        if curr.startswith("I-") and prev == "O":
            fixed[i] = "B-" + curr[2:]
    return fixed
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(len(input_ids)):
                ...
                all_preds.append(fix_illegal_bio(pred_seq) if use_crf else pred_seq)
