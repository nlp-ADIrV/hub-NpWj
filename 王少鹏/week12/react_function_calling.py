...
MAX_HISTORY_MESSAGES = 40  # 滑动窗口：保留 system + 最近 N 条消息


def _trim_messages(messages: list, max_count: int = MAX_HISTORY_MESSAGES) -> list:
    """裁剪对话历史，保留 system prompt + 最近 max_count 条消息"""
    if len(messages) <= max_count:
        return messages
    system = messages[:1]
    recent = messages[-(max_count - 1):]
    return system + recent


def run(question: str, max_steps: int = 10,
        messages: list | None = None) -> Generator[dict, None, None]:
    """
    执行 Function Calling 版 ReAct 循环，yield 每一步结构化结果

    参数：
      messages: 可选，外部传入的对话历史列表（会被原地修改）。
                若为 None 则新建；若传入已有列表则追加新问题，实现多轮对话。
...
def run_and_print(question: str, max_steps: int = 10, messages: list | None = None):
    print(f"\n{'='*60}")
    print(f"问题: {question}")
