...
教学重点：
  同一组任务，主 agent 派发的 subagent 分别用「并行(ThreadPool)」和
...
EVAL_QUESTIONS = [
    "分析 workspace/notes 目录下 3 个文件的要点",
    "中国咖啡市场调研：市场规模、主要品牌、消费趋势",
    "对 workspace/projects 目录下 2 个产品简介文件做竞品分析",
...
def run_one(question, serial):
    ...
    import agents
    t0 = time.time()
    r = agents.run_task(question, serial=serial)
    wall = time.time() - t0
