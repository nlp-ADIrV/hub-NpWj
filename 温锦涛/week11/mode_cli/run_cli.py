...
NAMED_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "可查 A 股年报（rag_search/list_companies）和天气（weather/weather_geocode/weather_by_coords）。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
                        ...
                        "description": "rag_search（查年报，需 query+可选 stock_code/year/top_k）/"
                                       ...
                                       " weather（查天气，需 city，一步到位）/"
                                       " weather_geocode（查城市坐标，需 city）/"
                                       " weather_by_coords（根据坐标查天气，需 lat+lon）",
                    },
                    "args": {
                        "type": "object",
                        "description": "命令参数。rag_search: {query, stock_code?, year?, top_k?}; weather: {city}; weather_geocode: {city}; weather_by_coords: {lat, lon}",
                    },
                },
                "required": ["command"],
            },
...
SYSTEM_PROMPT_NAMED = (
    ...
    "不在库内的公司请明确告知，不要臆测。天气相关命令：weather（一步到位）/ weather_geocode（查坐标候选）/ weather_by_coords（据坐标查天气）。"
    "每一轮可一次调用多个工具。如果信息不够，可在下一轮继续调用工具。"
)
...
SYSTEM_PROMPT_BASH = (
    ...
    "查天气：fincli weather --city 南京（一步到位）；"
    "fincli geocode --city 宁德（查坐标候选）；"
    "fincli weather-by-coords --lat 26.66 --lon 119.52（据坐标查天气）。"
    "回答必须依据命令返回的原文，不要编造。知识库仅含 5 家公司（茅台/五粮液/宁德时代/海康威视/中国平安），"
    "不在库内的明确告知。每一轮可一次调用多个工具。如果信息不够，可在下一轮继续调用工具。"
)
...
def run(client, model: str, question: str, mode: str, verbose: bool = True) -> dict:
    ...
    while turn < MAX_TURNS:
        ...
        for tc in msg.tool_calls:
            ...
            tool_call_log.append({"name": tc.function.name, "args": args, "turn": turn})
            tool_names.append(f"{tc.function.name}({args})")
            if verbose:
