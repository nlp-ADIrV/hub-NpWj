...
def _geocode_raw(client: httpx.Client, name: str) -> list[dict]:
    """内部：调用 Open-Meteo Geocoding API，返回原始结果列表。"""
...
def geocode(city: str) -> str:
    ...
    Returns:
        格式化候选列表，每条含行政级别、人口、坐标
    """
    with httpx.Client(timeout=10.0) as client:
        results = _geocode_raw(client, city)
        if not results:
            return f"未找到城市 '{city}'"

        lines = [f"找到 {len(results)} 个候选："]
        for i, r in enumerate(results, 1):
            ...
            details = [fc]
            if pop:
                details.append(f"人口 {pop}")
            loc_parts = [p for p in [name, admin1, country] if p]
            if len(loc_parts) == 3:
                location = f"{country} {admin1} {name}"
            elif len(loc_parts) == 2:
                location = f"{admin1} {name}"
            else:
                location = name
            lines.append(
                f"  候选 {i}：{location} ({', '.join(details)}) → {lat:.2f}°N, {lon:.2f}°E"
            )
...
def get_weather(city: str) -> str:
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        if is_low_admin and not has_suffix:
            retry = _geocode_raw(client, city + "市")
            if retry:
                results = retry
        ...
        # Step 2：天气查询
        weather_report = get_weather_by_coords(lat, lon)
        if weather_report.startswith("天气数据获取失败"):
            return weather_report

        return f"【{location_str}】天气报告\n{weather_report}"
