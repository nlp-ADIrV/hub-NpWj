...
def summarize(data: dict, question: str) -> dict:
    ...
    # 幻觉拒绝判定：问题涉及不在库的公司且答案含拒绝词
    need_refuse = any(kw in question for kw in NEED_REFUSE_KEYWORDS)
    refused = any(p in answer for p in REFUSE_PATTERNS) if need_refuse else None
    # 多轮拒绝标注
    if refused and turns and turns > 1:
        refused_label = f"✓拒绝（多调{turns - 1}轮）"
    elif refused:
        refused_label = "✓拒绝"
    elif refused is False:
        refused_label = "✗未拒绝（可能幻觉）"
    else:
        refused_label = "-"
    return {
        "tools": tool_names,
...
def run_compare(questions: list[str], provider: str) -> list[dict]:
    ...
    for qi, q in enumerate(questions, 1):
        ...
        for mode_name, mode_cmd in MODES:
            ...
            turns = data.get("turns", 1)
            print(f"{status} 工具[{s['tool_count']}] 轮数[{turns}] {s['llm_elapsed']}")
            rows.append({
...
def write_markdown(rows: list[dict], questions: list[str], provider: str, path: Path):
    lines = [
        ...
        "## 对比表",
        "",
        "| 问题 | 方式 | 工具调用 | 工具数 | 轮数 | LLM调用 | LLM耗时 | 拒绝标注 | 答案摘要 |",
        "|------|------|---------|:-----:|:----:|:-------:|:-------:|:--------:|---------|",
    ]
    for r in rows:
        lines.append(
            f"| {r['question']} | {r['mode']} | {r['tools']} | {r['tool_count']} | "
            f"{r['turns']} | {r['llm_calls']} | {r['llm_elapsed']} | {r.get('refused_label', '-')} | {r['answer_preview']} |"
        )
    ...
    lines += [
        ...
        "- **多轮循环**：三种 runner 均支持 while 循环（MAX_TURNS=5），模型可自主决定何时停止调工具并回答。"
        "轮数列和 LLM 调用数列反映模型的链式推理深度。",
        "- **天气工具拆分**：`get_weather`（一步到位）与 `geocode` + `get_weather_by_coords`（分步链式）并存。"
        "分步时 LLM 可看到 geocoding 候选列表并自行决策。",
        ...
        "CLI 与模型完全无关。",
        "- **幻觉控制**：问比亚迪（不在知识库）时，看各方式是否正确拒绝、是否多调工具后拒绝——"
        "多调工具也是一种模型策略差异。",
        "",
        "## 各方式原始回答",
        "",
...
def main():
    ...
    # 控制台简表
    print(f"\n{'='*80}\n对比表（已写入 {out_path}）\n{'='*80}")
    print(f"{'问题':<28}{'方式':<16}{'工具数':<6}{'轮数':<6}{'LLM调用':<8}{'LLM耗时':<8}{'拒绝':<8}")
    print("-" * 80)
    for r in rows:
        refuse = "-" if r["refused"] is None else ("✓" if r["refused"] else "✗")
        print(f"{r['question'][:26]:<28}{r['mode']:<16}{r['tool_count']:<6}{r['turns']:<6}{r['llm_calls']:<8}{r['llm_elapsed']:<8}{refuse:<8}")
