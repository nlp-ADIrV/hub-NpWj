...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。每次你可以一次调用多个工具。"
    "如果检索结果不够精确或信息不足，你可以再次调用工具获取更多信息，"
    "直到信息足够为止。"
)


def run(client, model: str, question: str, verbose: bool = True, max_rounds: int = 5) -> dict:
    """
    多轮循环（ReAct）：提问 → 模型输出 tool_call → 执行 → 回填 → 再次请求 → ...
    直至模型不再调用工具（给出最终回答）或达到 max_rounds 限制。
    与单轮闭环的区别：工具结果回填后会再次请求 LLM，LLM 可以基于结果
    决定"信息不够，再调一次"——这正是 Agent 多轮循环的核心。
    ...
    # ── 多轮循环：模型可以反复调工具，直到它觉得信息足够 ──
    while round_count < max_rounds:
        round_count += 1
        if verbose and round_count > 1:
            print(f"  → [loop] 第 {round_count} 轮 LLM 请求...")
        ...
        # 没有 tool_calls → LLM 认为信息够了，给出最终回答，退出循环
        if not msg.tool_calls:
            answer = msg.content or ""
            break

        # 有 tool_calls → 逐个执行后端函数
        messages.append(msg)
        ...
        # 工具结果已回填到 messages，下一轮循环 LLM 可以基于结果决定是否继续调工具

    # 达到最大轮数但 LLM 仍未给出最终回答
    if round_count >= max_rounds and not answer:
        answer = (f"[已达到最大循环轮数 {max_rounds}，强制结束]\n"
                  f"最后一轮 LLM 回复：{msg.content or '(无内容)'}")

    elapsed = time.time() - t0
    if verbose:
        print(f"  → [llm] 最终回答（{elapsed:.1f}s，共 {round_count} 轮）")
...
def main():
    ...
    parser.add_argument("--max-rounds", type=int, default=5, help="最大循环轮数，默认5")
