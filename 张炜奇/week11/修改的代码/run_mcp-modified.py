...
教学重点：
  ...
  3. 多轮循环（ReAct）代码和 run_function_call.py 几乎一样——差异只在"工具从哪来/怎么执行"
...
async def run(client, model: str, question: str,
    ...
    """多轮循环（ReAct）：提问 → 模型输出 tool_call → 路由到 Server 执行 → 回填 → 再次请求 → ...
    直至模型不再调用工具或达到 max_rounds 限制。"""
    messages = [
    ...
    while round_count < max_rounds:
        ...
        if verbose and round_count > 1:
            print(f"  → [mcp loop] 第 {round_count} 轮 LLM 请求...")
    ...
    async with AsyncExitStack() as stack:
        ...
        for i, q in enumerate(questions, 1):
            ...
            result = await run(client, model, q, tool_registry, openai_tools,
                               verbose=verbose and not as_json, max_rounds=max_rounds)
            result["question"] = q
