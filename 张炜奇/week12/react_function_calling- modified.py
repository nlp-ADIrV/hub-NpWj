...
教学重点：
  ...
  5. 多轮对话：通过 messages 持久化实现上下文记忆，支持连续追问

使用方式：
  python react_function_calling.py                              # 单轮问答
  python react_function_calling.py --chat                       # 多轮对话模式 ⭐
...
def run(question: str, max_steps: int = 10, messages: list | None = None) -> Generator[dict, None, None]:
    ...
    参数:
      question:   用户问题
      max_steps:  最大推理步数
      messages:   可选，已有的对话历史列表。传入时会在其基础上追加本轮问题并继续对话，
                  实现多轮上下文记忆。不传则创建全新的单轮对话。
    """
...
def chat(max_steps: int = 10):
    """
    交互式多轮对话模式

    与单轮 run() 的核心区别：
      - messages 列表在多次问答间持久化，实现上下文记忆
      - 每轮结束后将 Final Answer 作为 assistant 消息追加到 messages
      - 下一轮的问题作为新的 user 消息追加，模型能看到完整历史
    ...
    print(f"   输入 'exit' / 'quit' 退出对话")
    print(f"   输入 'history' 查看当前对话轮数")
    print(f"{'='*60}")
    ...
    while True:
        ...
        if question.lower() in ("exit", "quit", "q"):
            print(f"👋 对话结束，共进行 {turn} 轮")
            break

        if question.lower() == "history":
            print(f"📋 当前已进行 {turn} 轮对话，上下文消息数: {len(messages)}")
            continue
        ...
        turn += 1
        print(f"🧠 Agent 思考中...", end="", flush=True)
        ...
        for step_data in run(question, max_steps=max_steps, messages=messages):
            ...
            if stype == "action":
                action_count += 1
                obs_preview = step_data["observation"][:150].replace("\n", " ")
                if len(step_data["observation"]) > 150:
                    obs_preview += "..."
                print(f"\r  [Step {step_data['step']}] 🔧 {step_data['action']}"
                      f"({json.dumps(step_data['action_input'], ensure_ascii=False)})")
                print(f"    👁 {obs_preview}")

            elif stype == "final":
                elapsed = time.time() - start
                print(f"\r{'─'*50}")
                print(f"  ✅ Agent: {step_data['answer']}")
                print(f"  ⏱ {action_count} 步工具调用，耗时 {elapsed:.1f}s")

            elif stype == "max_steps":
                print(f"\r  ⚠️ {step_data['answer']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Function Calling 版 ReAct Agent（支持多轮对话）")
    parser.add_argument("--question",  default="贵州茅台和五粮液2023年的毛利率哪家更高？差多少个百分点？")
    parser.add_argument("--max_steps", type=int, default=10)
    parser.add_argument("--chat", action="store_true", help="进入多轮对话模式，支持连续追问和上下文记忆")
