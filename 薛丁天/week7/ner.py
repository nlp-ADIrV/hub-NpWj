"""
人民日报 NER 训练脚本（BERT + Linear / CRF）

基于 src/train.py 适配 peoples_daily 数据集（CoNLL 格式，3类实体：PER/ORG/LOC）

与 train.py 的区别：
  - 数据格式：peoples_daily 已是 BIO 格式，无需 span_to_bio 转换
  - 标签体系：7 个标签（O + B-PER/I-PER + B-ORG/I-ORG + B-LOC/I-LOC）
  - 训练数据：约 20,864 条/集（远多于 cluener 的 10,748 条）
...
from dataset_peoples_daily import (
    load_label_schema,
    build_peoples_daily_dataloaders,
)
...
def main():
    ...
    # 标签体系（从 peoples_daily/label_names.json 加载）
    labels, label2id, id2label = load_label_schema()
    ...
    # DataLoader（peoples_daily 数据集）
    train_loader, val_loader, _ = build_peoples_daily_dataloaders(
        tokenizer=tokenizer,
        label2id=label2id,
    ...
    run_tag = f"peoples_daily_{'crf' if args.use_crf else 'linear'}"
...
def parse_args():
    parser = argparse.ArgumentParser(description="训练人民日报 NER 模型（BERT + Linear/CRF）")
