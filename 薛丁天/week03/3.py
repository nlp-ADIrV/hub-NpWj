...
模型：Embedding → RNN/LSTM → 最后一个隐藏状态 → Linear → Softmax
...
# ─── 4. 模型定义 ────────────────────────────────────────────
class RNNCatModel(nn.Module):
    """RNN 版本：Embedding → RNN → Linear → Softmax"""
    ...
    def forward(self, x):
        # x: (batch, seq_len=5)
        e, hidden = self.rnn(self.embedding(x))    # e: (B, 5, hidden), hidden: (1, B, hidden)
...
def main():
    ...
    # ── 训练 RNN 模型 ──
    rnn_model = RNNCatModel(vocab_size=len(vocab))
    rnn_model = train_model(rnn_model, train_loader, val_loader, name="RNN")
    ...
    print(f"\n{'文本':<12} | {'RNN预测':<8} | {'LSTM预测':<8} | {'实际位置':<8}")
    print("-" * 50)
    for text in test_cases:
        rnn_pred, rnn_prob = predict(rnn_model, text, vocab)
        lstm_pred, lstm_prob = predict(lstm_model, text, vocab)
        actual = list(text).index('你')
        rnn_ok = "✓" if rnn_pred == actual else "✗"
        lstm_ok = "✓" if lstm_pred == actual else "✗"
        print(f"{text:<12} | {rnn_pred}{rnn_ok} ({rnn_prob[rnn_pred]:.2f}) | {lstm_pred}{lstm_ok} ({lstm_prob[lstm_pred]:.2f}) | {actual}")
