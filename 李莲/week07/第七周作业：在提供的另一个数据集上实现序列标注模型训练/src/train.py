...
使用方式：
  ...
  python train.py --use_crf              # 训练 BERT+CRF（推荐医疗领域）
  python train.py --epochs 3 --lr 2e-5  # 自定义超参数
...
BERT_PATH = BERT_PATH = "bert-base-chinese"
# ====================== 修改1：默认数据集路径改为医疗数据集 ======================
...
def evaluate_epoch(
    ...
    from seqeval.metrics import f1_score as seqeval_f1, precision_score, recall_score
    ...
    entity_precision = precision_score(all_golds, all_preds)
    entity_recall = recall_score(all_golds, all_preds)
    
    # 新增：打印每个实体类别的详细指标（医疗领域更关注单类性能）
...
def main():
    ...
    print(f"实体类型：{[l[2:] for l in labels if l.startswith('B-')]}")
    ...
    print(f"有效批量大小：{args.batch_size * args.grad_accum}")

    run_tag = "medical_crf" if args.use_crf else "medical_linear"
    ...
    # 训练结束后自动在测试集上评估
    print(f"\n{'='*50}")
    print(f"训练完成！最优 val_entity_f1={best_f1:.4f}")
    print(f"  Checkpoint: {ckpt_path}")
    print(f"  训练日志:   {log_path}")
    
    print("\n正在测试集上评估最终性能...")
    test_loss, test_f1 = evaluate_epoch(model, test_loader, id2label, device, args.use_crf)
    print(f"测试集最终结果：test_loss={test_loss:.4f}, test_entity_f1={test_f1:.4f}")


def parse_args():
    parser = argparse.ArgumentParser(description="训练 BERT NER 模型（医疗数据集）")
    parser.add_argument("--use_crf", action="store_true", help="使用 CRF 层（医疗领域推荐开启）")
    parser.add_argument("--bert_path", type=Path, default=BERT_PATH)
    # ====================== 修改2：针对医疗小数据集优化默认超参数 ======================
    parser.add_argument("--epochs", type=int, default=2, help="小数据集不要超过3轮，防止过拟合")
    parser.add_argument("--batch_size", type=int, default=8, help="单卡batch_size，配合梯度累积")
    parser.add_argument("--max_length", type=int, default=128, help="医疗文本长度适中")
    ...
    parser.add_argument("--warmup_ratio", type=float, default=0.2, help="小数据集增大预热比例")
    parser.add_argument("--grad_accum", type=int, default=2, help="梯度累积，有效batch_size=16")
    parser.add_argument("--dropout", type=float, default=0.2, help="增大dropout防止过拟合")
