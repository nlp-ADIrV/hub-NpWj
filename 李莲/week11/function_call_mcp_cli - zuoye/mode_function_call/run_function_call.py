...
与其它方式的关系：
  本文件的 LLM 循环调用代码，和 mode_mcp/run_mcp.py、mode_cli/run_cli.py 核心逻辑一致，
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。你可以一次调用多个工具，也可以根据工具返回结果继续调用新的工具。"
)


def run(client, model: str, question: str, verbose: bool = True, max_turns: int = 5) -> dict:
    """
    多轮循环工具调用：提问 → 模型决策是否调用工具 → 执行工具 → 回填结果 → 循环，直到无工具调用或达最大轮次。
    返回 {answer, tool_calls, elapsed} 用于对比器汇总，结构完全兼容原版本。
    
    :param client: OpenAI 兼容客户端实例
    :param model: 模型名称
    :param question: 用户问题
    :param verbose: 是否打印详细执行日志
    :param max_turns: 最大工具调用轮次，防止死循环，默认5轮
    """
    messages = [
    ...
    while current_turn < max_turns:
        ...
        # 终止条件：模型未发起工具调用，退出循环，直接返回回答
        if not msg.tool_calls:
            if verbose:
                print(f"  → [第 {current_turn} 轮] 无工具调用，生成最终回答")
            break

        # 将助手的工具调用指令回填到对话上下文，保持对话完整性
        messages.append(msg)

        if verbose:
            print(f"  → [第 {current_turn} 轮] 执行 {len(msg.tool_calls)} 个工具：")

        # 批量执行本轮所有工具调用（支持并行多工具）
        for tc in msg.tool_calls:
            ...
            # 以 role=tool 将结果回填到上下文，tool_call_id 必须与调用对应
            messages.append({
                "role": "tool",
                "tool_call_id": tc.id,
                "name": name,
    ...
    # 达到最大轮次仍有工具调用时的提示
    if current_turn >= max_turns and msg.tool_calls:
        if verbose:
            print(f"\n  ⚠ 已达到最大调用轮次上限（{max_turns}轮），强制结束调用")

    answer = msg.content or ""
    elapsed = time.time() - t0
    if verbose:
        print(f"\n  → [执行完成] 总耗时 {elapsed:.1f}s，累计调用工具 {len(tool_call_log)} 次")
...
DEMO_QUESTIONS = [
    ...
    # 多轮调用示例：先查公司列表，再查年报
    "先列出知识库中所有的公司，再查海康威视2022年的研发投入。",
]


def main():
    import argparse
    parser = argparse.ArgumentParser(description="方式一：Function Call（支持多轮循环调用）")
    ...
    parser.add_argument("--max-turns", type=int, default=5, help="最大工具调用轮次，默认5")
    args = parser.parse_args()

    client, model = build_client(args.provider)
    if not args.json:
        print(f"[Function Call] provider={args.provider} model={model} 最大轮次={args.max_turns}\n")
    ...
    for i, q in enumerate(questions, 1):
        if not args.json:
            ...
            print(f"Q{i}：{q}")
            print("=" * 60)
        result = run(client, model, q, verbose=not (args.quiet or args.json), max_turns=args.max_turns)
        result["question"] = q
